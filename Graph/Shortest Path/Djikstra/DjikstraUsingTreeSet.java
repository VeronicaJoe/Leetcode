class Solution {
    public ArrayList<Integer> dijkstra(int V, int[][] edges, int src) {
        ...
        {int cmp = Integer.compare(a[0],b[0]);
            if(cmp!=0)
            return cmp;//different dist
        return Integer.compare(a[1],b[1]);//if same dist compare with node val
        });//min heap
        ...
        while(!set.isEmpty())
        {
            int[] popped = set.pollFirst();//smallest
            ...
            for(int[] neighbor:adj.get(nodeVal))
            {
                ...
                {
                    set.remove(new int[]{dist[neighborVal],neighborVal});
                    dist[neighborVal]=nodeDist+edgeWeight;
                    set.add(new int[]{dist[neighborVal],neighborVal});
                }
            }
        }
